...
function _loadGids(buf) {
  ...
  if (self._encoding === 'csv') {
    String(buf).split(',').forEach(function (i) {
      self._addTile(parseInt(i, 10))
    })
  } else {
    for (var i = 0, len = buf.length / 4; i < len; i++) {
      self._addTile(buf.readUInt32LE(i * 4))
    }
  }
...
// Internal use only.
//
// Synchronously adds a new Layer as the new topmost Layer.
//
...
//
// ## _addTile `_addTile(gid)`
//
// Internal use only.
//
// Synchronously adds **gid** to the currently topmost Layer.
//
Parser.prototype._addTile = _addTile
function _addTile(gid) {
  var self = this

  self.layers[self.layers.length - 1].data.push(Number(gid))
